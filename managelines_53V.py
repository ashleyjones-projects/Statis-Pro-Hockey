...
def manage_lines(Linvf,Linvd,Linhf,Linhd,stamina,line_seq,result,mode,s_player_type,puckposs,line_ticker,penbox):
    ...
    # HOME PK
    if Linhf["NUM"] == 1:
        hfs_avg = (stamina["3KCH1"]["3KCH1"][3])/1 
    elif Linhf["NUM"] == 2:
        hfs_avg = (stamina["3KCH2"]["3KCH2"][3])/1

    if Linhd["NUM"] == 1:
        hds_avg = (stamina["3KLDH1"]["3KLDH1"][3] + stamina["3KRDH1"]["3KRDH1"][3])/2 
    elif Linhd["NUM"] == 2:
        hds_avg = (stamina["3KLDH2"]["3KLDH2"][3] + stamina["3KRDH2"]["3KRDH2"][3])/2
    ...
    if mode["HOME"] ==0 and mode["VISITOR"] == 0: # PURE AI
       ...
       if "ICING_0" in s_player_type or "ICING_1" in s_player_type:
                ...
                if (stamina["3KCH1"]["3KCH1"][3])/1 > 60:
                       line_seq["LINHF"] = 1
                else:
                       line_seq["LINHF"] = 2
                
                
                if (stamina["3KLDH1"]["3KLDH1"][3] + stamina["3KRDH1"]["3KRDH1"][3])/2 > 60:
                       line_seq["LINHD"] = 1
                else:
                       line_seq["LINHD"] = 2
