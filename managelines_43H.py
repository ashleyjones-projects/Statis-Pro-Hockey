...
def manage_lines(Linvf,Linvd,Linhf,Linhd,stamina,line_seq,result,mode,s_player_type,puckposs,line_ticker,penbox):
    ...
    # HOME PK
    if Linhf["NUM"] == 1:
        vfs_avg = (stamina["3KCV1"]["3KCV1"][3])/1 
    elif Linhf["NUM"] == 2:
        vfs_avg = (stamina["3KCV2"]["3KCV2"][3])/1

    if Linhd["NUM"] == 1:
        vds_avg = (stamina["3KLDV1"]["3KLDV1"][3] + stamina["3KRDV1"]["3KRDV1"][3])/2 
    elif Linhd["NUM"] == 2:
        vds_avg = (stamina["3KLDV2"]["3KLDV2"][3] + stamina["3KRDV2"]["3KRDV2"][3])/2
    ...
    if mode["HOME"] ==0 and mode["VISITOR"] == 0: # PURE AI
       ...
       if "ICING_0" in s_player_type or "ICING_1" in s_player_type:
                ...
                if (stamina["3KCV1"]["3KCV1"][3])/1 > 60:
                       line_seq["LINVF"] = 1
                else:
                       line_seq["LINVF"] = 2
                
                
                if (stamina["3KLDV1"]["3KLDV1"][3] + stamina["3KRDV1"]["3KRDV1"][3])/2 > 60:
                       line_seq["LINVD"] = 1
                else:
                       line_seq["LINVD"] = 2
