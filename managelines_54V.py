...
def manage_lines(Linvf,Linvd,Linhf,Linhd,stamina,line_seq,result,mode,s_player_type,puckposs,line_ticker,penbox):
    ...
    # VISITOR PP
    if Linvf["NUM"] == 1:
        vfs_avg = (stamina["5LWV1"]["5LWV1"][3] + stamina["5CV1"]["5CV1"][3] + stamina["5RWV1"]["5RWV1"][3])/3           
    elif Linvf["NUM"] == 2:
        vfs_avg = (stamina["5LWV2"]["5LWV2"][3] + stamina["5CV2"]["5CV2"][3] + stamina["5RWV2"]["5RWV2"][3])/3   
    
    if Linvd["NUM"] == 1:
        vds_avg = (stamina["5LDV1"]["5LDV1"][3] + stamina["5RDV1"]["5RDV1"][3])/2  
    elif Linvd["NUM"] == 2:
        vds_avg = (stamina["5LDV2"]["5LDV2"][3] + stamina["5RDV2"]["5RDV2"][3])/2  
   
    # HOME PK
    if Linhf["NUM"] == 1:
        hfs_avg = (stamina["4KLWH1"]["4KLWH1"][3] + stamina["4KCH1"]["4KCH1"][3])/2 
    elif Linhf["NUM"] == 2:
        hfs_avg = (stamina["4KLWH2"]["4KLWH2"][3] + stamina["4KCH2"]["4KCH2"][3])/2

    if Linhd["NUM"] == 1:
        hds_avg = (stamina["4KLDH1"]["4KLDH1"][3] + stamina["4KRDH1"]["4KRDH1"][3])/2 
    elif Linhd["NUM"] == 2:
        hds_avg = (stamina["4KLDH2"]["4KLDH2"][3] + stamina["4KRDH2"]["4KRDH2"][3])/2
    ...
    if mode["HOME"] ==0 and mode["VISITOR"] == 0: # PURE AI
       ...
       if "ICING_0" in s_player_type or "ICING_1" in s_player_type:
                ...
                if (stamina["4KLWH1"]["4KLWH1"][3] + stamina["4KCH1"]["4KCH1"][3])/2 > 60:
                       line_seq["LINHF"] = 1
                else:
                       line_seq["LINHF"] = 2
                
                
                if (stamina["4KLDH1"]["4KLDH1"][3] + stamina["4KRDH1"]["4KRDH1"][3])/2 > 60:
                       line_seq["LINHD"] = 1
                else:
                       line_seq["LINHD"] = 2
